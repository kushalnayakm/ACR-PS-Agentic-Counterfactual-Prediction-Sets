...
def marker_points(strict_alphas_avg_acc_se_df):
    ...
    matching_alphas = alg_alphas_df[(run_q)&(alg_q)]['Alpha value'].values
    if len(matching_alphas) == 0:
        # Fallback if no matching algorithm results 
        alpha_ours = strict_alphas_avg_acc_se_df['avg_strict'].idxmax()
    else:
        # Use requested seed if possible, otherwise first available
        seed_idx = config.run_no_seed if config.run_no_seed < len(matching_alphas) else 0
        alpha_ours = matching_alphas[seed_idx]
